...
#----------------------------------------------------------------------
#- Draw a map of the cases and the population census
...
census_mapd <- data.frame(Longitude = census_coords3$h07_gpslon, Latitude = census_coords3$h07_gpslat,
                            Key = "Census")
cases_mapd <- data.frame(Longitude = newd2$Longitude, Latitude = newd2$Latitude, Key = "Case")
hc_mapd <- data.frame(Longitude = 35.0395908, Latitude = -15.7780157, Key = "Ndirande HC" )
...
ggmap(ndirande_terrain) + geom_point(data = map_d, 
                                       ...
                                       size = 1) +
  scale_color_manual(values = c("Ndirande HC" = "black", "Case" = "red"))+
...
ggmap(ndirande_terrain) + geom_point(data = map_d2, 
                                     aes(x = Longitude, y = Latitude, col = Key), 
                                     size = 0.5) +
  scale_color_manual(values = c("Ndirande HC" = "black", "Census" = "red"))+
  labs(x = "Longitude", y = "Latitude") +
  theme(plot.title = element_text(hjust = 0.5),
        axis.text = element_text(size = 12, color= "black"),
        axis.title=element_text(size=14, colour = "black", face = "plain"),
...
Ki <- Kinhom(x_typh, lam, correction = c("none"))
